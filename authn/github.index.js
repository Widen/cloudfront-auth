        ...
        if (responseQueryString.error) {
          internalServerError(
            'Error while getting token: ' +
              responseQueryString.error_description,
            callback
          )
        } else {
          const authorization =
            responseQueryString.token_type +
            ' ' +
            responseQueryString.access_token
          axios
              ...
              axios
                ...
                .then(function (response) {
                  ...
                  if (response.status === 204) {
                    ...
                    console.log(
                      'User not a member of required ORG. Unauthorized.'
                    )
                    unauthorized(
                      'Unauthorized. User ' +
                        response.login +
                        ' is not a member of required organization.',
                      callback
                    )
                  }
                })
        ...
        if (err) {
          switch (err.name) {
            ...
            default:
              ...
              unauthorized(
                'Unauthorized. User ' + decoded.sub + ' is not permitted.',
                callback
              )
          }
        } else {
