...
if (!fs.existsSync('distributions/' + DISTRIBUTION)) {
  console.log(
    colors.red(
      "Distribution '" + DISTRIBUTION + "' does not exist. Stopping test..."
    )
  )
  process.exit()
}
...
if (!fs.existsSync('distributions/' + DISTRIBUTION + '/config-test.json')) {
  console.log(
    colors.red(
      "config-test.json does not exist in '" +
        DISTRIBUTION +
        "'. Add config-test.json and restart. Stopping test..."
    )
  )
  process.exit()
}
...
var config = JSON.parse(
  fs.readFileSync('distributions/' + DISTRIBUTION + '/config.json', 'utf8')
)
var testConfig = JSON.parse(
  fs.readFileSync('distributions/' + DISTRIBUTION + '/config-test.json')
)
...
function setupRedirect(url, lambdaFunction) {
  ...
  prompt.get(
      ...
      properties: {
        wait: {
          description: colors.red(
            'Add ' +
              colors.green(url + config.CALLBACK_PATH) +
              ' to your list of redirect URIs. Press enter when finished'
          ),
          required: false,
        },
      },
    ...
    function (err, result) {
      ...
      // Update log
      fs.writeFileSync(
        'distributions/' + DISTRIBUTION + '/logs/' + logName + '.log',
          ...
          prompt.get(
            {
              properties: {
                code: {
                  description: colors.red(
                    'Enter the code in your browser received from authentication provider'
                  ),
                  required: false,
                },
              },
                ...
                if (err) {
                  ...
                  var params = {
                    ...
                    Payload: tokenRequestPayload(
                      url,
                      JSON.parse(data.Payload).headers['set-cookie'][0].value
                    ),
                    LogType: 'Tail',
                  }
                    ...
                    if (err) {
                      ...
                      fs.appendFileSync(
                        ...
                        '\n*/ Token Response /*\nStatus Code: ' +
                          ...
                          data.ExecutedVersion +
                          '\nLog Result:\n' +
                          new Buffer(data.LogResult, 'base64').toString(
                            'ascii'
                          ) +
                          '\nPayload:\n' +
                          beautify(JSON.parse(data.Payload), null, 2, 80)
        ...
        cf: {
          request: {
            ...
            querystring:
              'code=' +
              code +
              '&state=%2f&session_state=fc350b67-673e-4ecd-98e7-3c2f5a875d0a',
          },
          config: {
            distributionId: 'EXAMPLE',
            test: url,
