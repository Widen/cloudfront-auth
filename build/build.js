...
prompt.get(
  ...
  function (err, result) {
    ...
    if (
      fs.existsSync('distributions/' + config.DISTRIBUTION + '/config.json')
    ) {
      oldConfig = JSON.parse(
        fs.readFileSync(
          './distributions/' + config.DISTRIBUTION + '/config.json',
          'utf8'
        )
      )
    }
    if (
      !fs.existsSync('distributions/' + config.DISTRIBUTION + '/id_rsa') ||
      !fs.existsSync('./distributions/' + config.DISTRIBUTION + '/id_rsa.pub')
    ) {
      shell.exec(
        'ssh-keygen -t rsa -m PEM -b 4096 -f ./distributions/' +
          config.DISTRIBUTION +
          "/id_rsa -N ''"
      )
      shell.exec(
        'openssl rsa -in ./distributions/' +
          config.DISTRIBUTION +
          '/id_rsa -pubout -outform PEM -out ./distributions/' +
          config.DISTRIBUTION +
          '/id_rsa.pub'
      )
    }
...
function microsoftConfiguration() {
  ...
  prompt.get(
    ...
    function (err, result) {
      ...
      config.PUBLIC_KEY = fs.readFileSync(
        'distributions/' + config.DISTRIBUTION + '/id_rsa.pub',
        'utf8'
      )
      config.TENANT = result.TENANT
      config.DISCOVERY_DOCUMENT =
      ...
      shell.cp(
        './authz/microsoft.js',
        './distributions/' + config.DISTRIBUTION + '/auth.js'
      )
      shell.cp(
        './authn/openid.index.js',
        './distributions/' + config.DISTRIBUTION + '/index.js'
      )
      shell.cp(
        './nonce.js',
        './distributions/' + config.DISTRIBUTION + '/nonce.js'
      )
          ...
          prompt.get(
            {
              properties: {
                MOVE: {
                  message:
                    colors.red('Place ') +
                    colors.blue('google-authz.json') +
                    colors.red(' file into ') +
                    colors.blue('distributions/' + config.DISTRIBUTION) +
            ...
            function (err, result) {
              if (
                !shell.test(
                  '-f',
                  'distributions/' + config.DISTRIBUTION + '/google-authz.json'
                )
              ) {
                console.log(
                  'Need google-authz.json to use google groups authentication. Stopping build...'
                )
              } else {
                var googleAuthz = JSON.parse(
                  fs.readFileSync(
                    'distributions/' +
                      config.DISTRIBUTION +
                      '/google-authz.json'
                ...
                if (!googleAuthz.hasOwnProperty('cloudfront_authz_groups')) {
                  console.log(
                    'google-authz.json is missing cloudfront_authz_groups. Stopping build...'
                  )
                } else {
                  shell.cp(
...
function oktaConfiguration() {
  ...
  prompt.get(
    ...
    function (err, result) {
      ...
      config.DISCOVERY_DOCUMENT =
        result.BASE_URL + '/.well-known/openid-configuration'
      config.SESSION_DURATION = parseInt(result.SESSION_DURATION, 10) * 60 * 60
      ...
      if (result.CLIENT_SECRET) {
        ...
        config.PKCE_CODE_VERIFIER_LENGTH =
          result.PKCE_CODE_VERIFIER_LENGTH || '96'
        shell.cp(
          './code-challenge.js',
          './distributions/' + config.DISTRIBUTION + '/code-challenge.js'
        )
        shell.cp(
          './authn/pkce.index.js',
          './distributions/' + config.DISTRIBUTION + '/index.js'
    ...
    function (err, result) {
      axios
        ...
        .then(function (response) {
          if (response.status == 200) {
            ...
            config.SESSION_DURATION =
              parseInt(result.SESSION_DURATION, 10) * 60 * 60
            config.CALLBACK_PATH = url.parse(result.REDIRECT_URI).pathname
            ...
            console.log(
              'Organization could not be verified (code ' +
                response.status +
                '). Stopping build...'
            )
          }
        ...
        .catch(function (error) {
          console.log(
            'Organization could not be verified. Stopping build... (' +
              error.message +
              ')'
          )
        })
...
function zip(files) {
  ...
  for (var i = 0; i < files.length; i++) {
    filesString +=
      ' distributions/' + config.DISTRIBUTION + '/' + files[i] + ' '
  }
  shell.exec(
    'zip -q distributions/' +
      config.DISTRIBUTION +
      ...
      config.DISTRIBUTION +
      '.zip ' +
      'package-lock.json package.json -r node_modules'
  )
  shell.exec(
    'zip -q -r -j distributions/' +
      config.DISTRIBUTION +
      '/' +
      config.DISTRIBUTION +
      '.zip ' +
  ...
  console.log(
    colors.green(
      'Done... created Lambda function distributions/' +
        config.DISTRIBUTION +
        '/' +
        config.DISTRIBUTION +
        '.zip'
