...
function isAuthorized(
  ...
  axios
    .post(googleAuthz.token_uri, postData)
    .then(function (response) {
      for (var i = 0; i < googleAuthz.cloudfront_authz_groups.length; i++) {
        ...
        var membershipGet =
          ...
          googleAuthz.cloudfront_authz_groups[i] +
          '/hasMember/' +
          decoded.sub
        console.log(membershipGet + ': ' + authorization)
        axios
          .get(membershipGet, { headers: { Authorization: authorization } })
          .then(function (response) {
            groupChecks++
            if (
              !response.data.error &&
              ...
              groupChecks >= googleAuthz.cloudfront_authz_groups.length
            ) {
              unauthorized(
                'Unauthorized',
